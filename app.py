...
def lookup_pinellas_consolidated_zoning(address, city_name=None):
    ...
    try:
        ...
        if zoning_data.get('results') and len(zoning_data['results']) > 0:
            # Get first result (should be the zoning polygon containing the point)
            result = zoning_data['results'][0]
            ...
            return {
                'success': True,
                'zoning_code': zoning_code,
                'zoning_description': f"Zoning for {jurisdiction}" if jurisdiction else None,
                'future_land_use': None,  # PPC service may not have FLU - will add separately if needed
